...
public final class FlightClient {
    ...
    static void onClientTick(ClientTickEvent.Post event) {
        ...
        boolean rising = active == 1 && jumpHeldTicks >= Config.FLIGHT_HOLD_TICKS.get()
                && energy > 0.0F && !locked && !player.isSpectator();

        // Auto-hide the bar after 2s at max charge with no flying.
        if (energy >= 1.0F && !rising) {
            fullIdleTicks++;
        } else {
            fullIdleTicks = 0;
        }
        ...
        if (rising) {
            ...
            player.resetFallDistance(); // no fall build-up while you're actively rising
        } else {
            risingTicks = 0;
        }
        if (mode != lastMode) {
